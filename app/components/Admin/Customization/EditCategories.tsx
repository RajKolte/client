...
in error){
  ...
  const handleCategoriesAdd=(id:any, value:string) =>{
    setCategories ((prevCategory: any) => prevCategory.map((i: any) => (i._id
            === id? {...i, title: value } : i))
  }
  ...
  const areCategoriesUnchanged =(
    ...
    newCategories: any []
        ...
        return JSON.stringify (originalCategories)
== JSON.stringify(newCategories) ;
    }

    const isAnyCategoryTitleEmpty = (categories:
        any []) => {
            return categories. some ((q) =>
            q.title === "");
            ...
            categories.map((item: any, index: number) => {
              ...
              <div key={item._id} className="p-3">
                <div className="flex items-center w-full justify-center">
                  ...
                  />
                  <AiOutlineDelete
                    className="dark:text-white text-black text-[18px] cursor-pointer ml-3"
                    onClick={() => {
...
onClick={newCategoriesHandler}/>
            ...
            ${
                areCategoriesUnchanged (data. layout.categories, categories) || isAnyCategoriesTitleEmpty(categories) ? "!cursor-not-allowed" : "!cursor-pointer !bg-[#42d383]"
            }
            !rounded absolute bottom-12 right-12`}
            onClick={
                areCategoriesUnchanged (data.layout.categories, categories) || isAnyCategoryTitleEmpty(categories) ? () => null : editCategoriesHandler } >Save
            </div>
        </div>
      )}
    </>
