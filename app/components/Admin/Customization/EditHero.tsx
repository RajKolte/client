...
const EditHero: FC<Props> = (props) => {
  ...
  useEffect(() => {
    if (data) {
      ...
      setImage(data?.banner.image?.url);
    }
    if (isSuccess) {
      refetch();
    ...
    if (file) {
      ...
      reader.onload = (e: any) => {
        if (reader.readyState === 2) {
          setImage(e.target.reader.result as string);
        }
      };
  ...
  <>
    <div className="w-full 1000px:flex items-center">
      <div className="absolute top-[100px] 1000px:top-[unset] 1500px:h-[700px] 1500px:w-[700px) 1100px:h-[500px] 1100px:w-[500px] ">
        <div
          className="1000px:w-[40%]
flex 1000px:min-h-screen items-center justify-end pt-[70px] 1000px:pt-[0] z-10"
        >
          <div className="relative flex items-center justify-end">
            <Image
              src={image}
              alt=""
              className="object-contain 1100px:max-w-[90%] w-[90%] 1500px:max-w-[85%]
h-[auto] z-[10]"
            />
            <input
              type="file"
